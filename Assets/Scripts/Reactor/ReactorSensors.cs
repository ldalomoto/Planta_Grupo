...
public class ReactorSensors : MonoBehaviour
{
    ...
    void UpdateCoreSensors()
    {
        ...
        foreach(FuelRod rod in fuelCore.fuelRods){
            ...
            if(rod.temperature > maxTemp) maxTemp = rod.temperature;
        }
        
        coreTemperature = maxTemp; // Usar temperatura máxima, no promedio
        neutronFlux /= fuelCore.fuelRods.Count;
    }

    void UpdateCoolantSensors()
    {
        // Modelo de presión más preciso
        float tempEffect = (coreTemperature - 573.15f) * 0.002f; // 300°C base
        float flowEffect = (1 - coolantFlowRate) * 0.5f;
        reactorPressure = 15.5f + tempEffect + flowEffect; // MPa
    }
